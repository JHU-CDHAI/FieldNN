...
class SoftmaxLayer(nn.Module):
    ...
    def forward(self, emission, targets):
        return self.loss_function(emission, targets)

    def decode(self, emission):
        # softmax: normalization
        probs = torch.nn.functional.softmax(emission, -1)
        pred_tags = torch.argmax(probs,-1)
        # pred_tags = pred_tags.masked_fill(leng_st_mask, 0).long()
        return pred_tags


class SoftmaxSumLayer(torch.nn.Module):
    """
    ...
    def __init__(self) -> None:
        super(SoftmaxSumLayer, self).__init__()

    def forward(self, emission: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        probs = torch.nn.functional.log_softmax(emission, dim=-1)
        return torch.nn.functional.nll_loss(probs, targets.long(), reduction = "sum")

    def decode(self, emission: torch.Tensor) -> torch.Tensor:
        probs = torch.nn.functional.softmax(emission,-1)
        pred_tags = torch.argmax(probs,-1)
        # pred_tags = pred_tags.masked_fill(leng_st_mask, 0).long()
        return pred_tags
...
class LogSoftmax(torch.nn.Module):
    def __init__(self, output_dim, n_class):
        ...
        :param n_class: int
        """
        self.hidden2out = torch.nn.Linear(output_dim, n_class)
        self.out2tag = torch.nn.LogSoftmax(dim=-1)
    ...
    def forward(self, input):
        ...
        :return:
        """
        out = self.hidden2out(input)
        tag = self.out2tag(out)
        return tag


# class SampledSoftmaxLayer(nn.Module):

#     def __init__(self, output_dim, n_class, n_samples):
#         """
...
#                                                                 None if chars_inp[0] is None else torch.LongTensor(
#                                                                     chars_inp).view(len(word_inp), 1, len(package[1])),
